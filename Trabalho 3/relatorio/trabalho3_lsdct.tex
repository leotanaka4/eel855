...
A equação característica do sistema em malha fechada é escrita como:
\begin{equation}
p_c(s) = \bar{M}s^3 + K_d s^2 + (K_d K_p + K_i)s + K_i K_p = 0
\end{equation}
em que essa equação foi derivada das equações \eqref{eq:pi_ppi}, \eqref{eq:p_ppi} e \eqref{eq:dinamica_simplificada}.
